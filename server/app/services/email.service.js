 ...
 */
exports.sendStudentGreeting = async (studentEmail, firstname, username, password) => {
  try {
    const info = await transporter.sendMail({
      from: `"IMS Management" <${process.env.MAIL_FROM || 'no-reply@ims.edu'}>`,
      to: studentEmail,
      subject: `Welcome to IMS, ${firstname}!`,
      text: `Hello ${firstname},\n\nWelcome to Institute Management System. Your account has been created.\n\nUsername: ${username}\nPassword: ${password}\n\nPlease login and change your password.\n\nBest regards,\nIMS Team`,
      html: `
        <div style="font-family: sans-serif; padding: 20px; color: #1e293b;">
          <h2 style="color: #6366f1;">Welcome to IMS, ${firstname}!</h2>
          <p>Your educational journey with us starts now. We've created your account with the following credentials:</p>
    ...
    logger.info(`Greeting email sent to ${studentEmail}: ${info.messageId}`);
    return true;
  } catch (error) {
    logger.error(`Failed to send email to ${studentEmail}: ${error.message}`);
    return false;
  }
};
