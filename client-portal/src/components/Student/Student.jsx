...
export const Student = () => {
  ...
  const [formData, setFormData] = useState({
    email: '', firstname: '', lastname: '', grade: '', contact: '', username: '', password: ''
  });
  ...
  const handleEdit = (id) => navigate(`/student/edit/${id}`);
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ email: '', firstname: '', lastname: '', grade: '', contact: '', username: '', password: '' });
      ...
      render: (id) => (
        <div className="flex items-center gap-3">
          <button 
            ...
            title={userRole === 'TUTOR' ? 'View Details' : 'Edit Student'}
          >
            <Edit3 size={18} />
            {userRole === 'TUTOR' && <span className="text-xs font-medium">Details</span>}
          </button>
          {userRole !== 'TUTOR' && (
