...
export const Chatroom = () => {
  ...
  const fetchSubjects = async (userId, role) => {
    ...
    try {
      ...
      if (role === 'TUTOR') {
        endpoint = `/tutors/subject-mapping/${userId}?type=user_id`;
      }
      const res = await api.get(endpoint);
      if (res.status === 200) {
        const subjectsData = role === 'TUTOR' ? res.data.subjects : res.data.data.subjects;
      ...
      <main className="flex-1 lg:ml-64 flex flex-col h-[calc(100vh-140px)] animate-in fade-in duration-500 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-[url('https://www.transparenttextures.com/patterns/dark-matter.png')]">
          {!selectedSubject ? (
            ...
            data.map((item) => {
              const isMine = userName === item.user.username;
              return (
