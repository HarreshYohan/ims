...
export const Staff = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Staff member ${modalMode === 'CREATE' ? 'created' : 'updated'}`);
      refetch();
    } catch (err) {
      const errorMsg = err.response?.data?.message || err.message || `Failed to ${modalMode === 'CREATE' ? 'create' : 'update'} staff`;
